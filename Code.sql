...
**Query #1**
    ...
    subset AS (
      SELECT
        ...
        MAX(txn_date) AS month_end_date
      FROM cte
      GROUP BY 1, 2
    ),
    final AS (
      ...
      LEFT JOIN cte b ON a.customer_id = b.customer_id
                    AND a.month_end_date = b.txn_date
    ),
    output AS (
      SELECT
        customer_id,
        txn_amount,
        month,
        LAG(txn_amount, 1) OVER (PARTITION BY customer_id ORDER BY month),
        100 * (txn_amount - LAG(txn_amount, 1) OVER (PARTITION BY customer_id ORDER BY month)) / LAG(txn_amount, 1) OVER (PARTITION BY customer_id ORDER BY month) AS change
      FROM final
      ORDER BY 1, 3
    )
    SELECT
      COUNT(DISTINCT CASE WHEN change > 5 THEN customer_id ELSE NULL END) * 100.0 / COUNT(DISTINCT customer_id)
    FROM output;
